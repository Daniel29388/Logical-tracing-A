...
_mootdx_client = None
_mootdx_server: tuple[str, int] | None = None
_mootdx_lock = threading.Lock()


def _parse_tdx_server(value) -> tuple[str, int] | None:
    """Parse a TDX server from ip:port strings or mootdx config rows."""
    if not value:
        return None
    ...
    if isinstance(value, dict):
        host = str(value.get("host") or value.get("ip") or value.get("address") or "")
        raw_port = value.get("port") or 7709
        ...
        if "," in text and ":" not in text:
            host, raw_port = [part.strip() for part in text.rsplit(",", 1)]
        elif ":" in text:
            host, raw_port = [part.strip() for part in text.rsplit(":", 1)]
        else:
            host = text
...
def _tdx_server_candidates() -> list[tuple[str, int]]:
    """Return TDX HQ servers without invoking mootdx bestip auto-detection."""
    candidates: list[tuple[str, int]] = []

    for env_name in ("TRADINGAGENTS_TDX_SERVER", "TDX_SERVER", "MOOTDX_SERVER"):
        parsed = _parse_tdx_server(os.environ.get(env_name))
        if parsed:
            candidates.append(parsed)

    config_path = os.path.join(os.path.expanduser("~"), ".mootdx", "config.json")
    try:
        with open(config_path, "r", encoding="utf-8") as fh:
            config = _json.load(fh)
        best_hq = (config.get("BESTIP") or {}).get("HQ")
        parsed = _parse_tdx_server(best_hq)
        if parsed:
            candidates.append(parsed)
        for item in ((config.get("SERVER") or {}).get("HQ") or []):
            parsed = _parse_tdx_server(item)
            if parsed:
                candidates.append(parsed)
    except Exception as exc:
        logger.debug("Unable to read mootdx config for TDX servers: %s", exc)
    ...
    try:
        ...
        logger.debug("Unable to load mootdx HQ_HOSTS: %s", exc)
...
def _tdx_timeout() -> int:
    try:
        return max(1, int(os.environ.get("TRADINGAGENTS_TDX_TIMEOUT", "5")))
    except ValueError:
        return 5


def _tdx_socket_timeout() -> float:
    try:
        return max(0.2, float(os.environ.get("TRADINGAGENTS_TDX_SOCKET_TIMEOUT", "1.2")))
    except ValueError:
        return 1.2


def _tdx_max_servers(total: int) -> int:
    try:
        configured = int(os.environ.get("TRADINGAGENTS_TDX_MAX_SERVERS", "12"))
    except ValueError:
        configured = 12
    return max(1, min(total, configured))


def _tcp_reachable(server: tuple[str, int], timeout: float | None = None) -> bool:
    try:
        with socket.create_connection(server, timeout=timeout or _tdx_socket_timeout()):
            return True
    except OSError:
        return False
...
def _probe_mootdx_client(client) -> bool:
    """Validate a TDX connection with a tiny daily-bar request."""
    try:
...
def _prefer_tdx_first() -> bool:
    return os.environ.get("TRADINGAGENTS_USE_TDX_FIRST", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "y",
...
def _get_mootdx_client():
    """Lazy-init mootdx Quotes client with explicit server probing."""
    global _mootdx_client, _mootdx_server
    ...
    with _mootdx_lock:
        ...
        errors: list[str] = []
        candidates = _tdx_server_candidates()
        for server in candidates[: _tdx_max_servers(len(candidates))]:
            if not _tcp_reachable(server):
                errors.append(f"{server[0]}:{server[1]} TCP closed/timeout")
                continue
            try:
                client = Quotes.factory(
                    market="std",
                    server=server,
                    timeout=_tdx_timeout(),
                )
                if _probe_mootdx_client(client):
                    _mootdx_client = client
                    _mootdx_server = server
                    logger.info("Connected to TDX server %s:%s", server[0], server[1])
                    return _mootdx_client
                errors.append(f"{server[0]}:{server[1]} returned no probe data")
                try:
                    client.close()
                except Exception:
                    pass
            except Exception as exc:
                errors.append(f"{server[0]}:{server[1]} {type(exc).__name__}: {exc}")

        sample = "; ".join(errors[:5])
        raise ConnectionError(
            "No reachable TDX HQ server. "
            "Set TRADINGAGENTS_TDX_SERVER=ip:port to pin a known-good server. "
            f"Tried {len(errors)} servers. Sample: {sample}"
        )
...
# ---------------------------------------------------------------------------
# HTTP K-line fallback helpers (direct HTTP, no akshare)
# ---------------------------------------------------------------------------
...
def _eastmoney_kline_fallback(
    ...
    """Fetch daily K-line from Eastmoney push2his HTTP API."""
...
# ---------------------------------------------------------------------------
# OHLCV loading with cache (Eastmoney/Sina HTTP -> mootdx fallback -> CSV)
# ---------------------------------------------------------------------------


def _mootdx_daily_bars(code: str) -> pd.DataFrame:
    client = _get_mootdx_client()
...
def _load_sina_then_mootdx(
    code: str,
    start_date: str | None = None,
    end_date: str | None = None,
) -> tuple[pd.DataFrame, str]:
    """Load OHLCV with configurable TDX priority and HTTP fallbacks."""
    prefer_tdx = _prefer_tdx_first()

    if prefer_tdx:
        try:
            df = _mootdx_daily_bars(code)
            if start_date:
                df = df[df["Date"] >= pd.to_datetime(start_date)]
            if end_date:
                df = df[df["Date"] <= pd.to_datetime(end_date)]
            if not df.empty:
                if _mootdx_server:
                    return df, f"mootdx TCP {_mootdx_server[0]}:{_mootdx_server[1]}"
                return df, "mootdx TCP"
        except Exception as exc:
            logger.info("TDX K-line failed for %s: %s, trying HTTP fallback", code, exc)

    try:
        df = _eastmoney_kline_fallback(code, start_date, end_date)
        if not df.empty:
            return df, "eastmoney push2his"
    except Exception as exc:
        logger.info("Eastmoney K-line failed for %s: %s, trying sina fallback", code, exc)
    ...
    try:
        ...
        logger.info("Sina K-line failed for %s: %s, trying mootdx fallback", code, exc)

    if prefer_tdx:
        raise ValueError(f"No OHLCV data from TDX/HTTP sources for {code}")

    df = _mootdx_daily_bars(code)
    return df, "mootdx TCP fallback"
...
def _load_ohlcv_astock(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    try:
        df, _ = _load_sina_then_mootdx(code)
        df = df[["Date", "Open", "High", "Low", "Close", "Volume"]]
        df["Date"] = pd.to_datetime(df["Date"])
    except Exception as exc:
        raise ValueError(f"No OHLCV data from TDX/HTTP sources for {code}: {exc}") from exc
...
def get_stock_data(
    ...
    """Get OHLCV stock price data via configured TDX/HTTP sources."""
    code = _normalize_ticker(symbol)

    try:
        df, data_source = _load_sina_then_mootdx(code, start_date, end_date)
    except Exception:
        return "K线数据获取失败：TDX/东方财富/新浪均不可用，请检查网络连接或设置 TRADINGAGENTS_TDX_SERVER"
...
# ---------------------------------------------------------------------------
# 18. Keyword / Concept Reverse Lookup (关键词 / 概念反查股票池)
# ---------------------------------------------------------------------------

def _split_keywords(keyword: str) -> list[str]:
    """Split a loose theme query into usable matching tokens."""
    tokens = _re.split(r"[,，/、|+>\-→\s]+", str(keyword or ""))
    return [t.strip() for t in tokens if t and t.strip()]


def _eastmoney_block_candidates() -> tuple[list[dict], list[str]]:
    """Fetch Eastmoney region / industry / concept block lists."""
    block_specs = [
        ("m:90+t:1", "地域板块"),
        ("m:90+t:2", "行业板块"),
        ("m:90+t:3", "概念板块"),
    ...
    for fs, label in block_specs:
        try:
            ...
            params = {
                ...
                "invt": "2",
                "fs": fs,
                "fields": "f2,f3,f4,f12,f13,f14,f20,f62,f104,f105,f128,f136,f140,f141",
            }
            r = _requests.get(url, params=params, headers={"User-Agent": _UA}, timeout=15)
            d = r.json()
    ...
    return [b for b in blocks if b.get("code") and b.get("name")], errors


def _match_blocks(keyword: str, max_blocks: int = 8) -> tuple[list[dict], list[str]]:
    """Match user keywords against Eastmoney block names and BK codes."""
    tokens = _split_keywords(keyword)
    if not tokens:
        return [], []

    blocks, errors = _eastmoney_block_candidates()
    matched: list[dict] = []
    ...
    for token in tokens:
        ...
        for block in blocks:
            ...
            if (
                ...
                if block["code"] in seen_codes:
                    continue
                item = block.copy()
                item["matched_keyword"] = token
                matched.append(item)
                seen_codes.add(block["code"])

    matched.sort(
        key=lambda b: (
            0 if b["name"] in tokens else 1,
            0 if b["type"] == "概念板块" else 1,
            b["name"],
        )
    )
    return matched[:max_blocks], errors


def _eastmoney_block_constituents(block_code: str, limit: int = 50) -> list[dict]:
    """Fetch stocks in an Eastmoney block, e.g. b:BK1038."""
    url = "https://push2.eastmoney.com/api/qt/clist/get"
    params = {
        "pn": "1",
        ...
        "invt": "2",
        "fs": f"b:{block_code}",
        "fields": "f2,f3,f4,f5,f6,f8,f10,f12,f13,f14,f20,f62,f100",
    }
    r = _requests.get(url, params=params, headers={"User-Agent": _UA}, timeout=15)
    d = r.json()
    rows = []
...
def _hot_stock_keyword_matches(keyword: str, curr_date: str, limit: int = 20) -> list[str]:
    """Fallback: filter today's hot-stock reason tags by keyword tokens."""
    tokens = _split_keywords(keyword)
    if not tokens:
        return []
    try:
        report = get_hot_stocks(curr_date)
    except Exception as e:
        logger.warning("Hot-stock fallback failed for %s: %s", keyword, e)
        return []
...
def get_stocks_by_keyword(
    keyword: Annotated[str, "Keyword, concept, industry, region, or BK code, e.g. 存储芯片 / 半导体设备 / 合肥 / BK1038"],
    curr_date: Annotated[str, "Date YYYY-MM-DD, empty string for today"] = "",
    top_n: Annotated[int, "Max constituent stocks per matched block"] = 50,
    max_blocks: Annotated[int, "Max matched blocks to expand"] = 8,
) -> str:
    """Reverse lookup A-share stock pools by keyword / concept / industry.

    First matches Eastmoney region, industry, and concept blocks by name or BK code,
    then expands each matched block into constituent stocks. If no block matches,
    falls back to today's hot-stock reason tags as discovery clues.
    """
    ...
    lines = [
        f"# Keyword Stock Pool: {keyword}",
        f"# Date: {curr_date}",
        "# Primary source: Eastmoney push2 block lists + block constituents",
        "",
    ]

    try:
        matched_blocks, block_errors = _match_blocks(keyword, max_blocks=max_blocks)
    except Exception as e:
        matched_blocks = []
        block_errors = [str(e)]
    ...
    if block_errors:
        ...
        for item in block_errors:
            lines.append(f"- Eastmoney block lookup warning: {item}")
        lines.append("")

    if matched_blocks:
        lines.append("## Matched Blocks")
        lines.append("| 类型 | 板块代码 | 板块名称 | 匹配词 | 涨跌幅 | 上涨/下跌家数 | 领涨股 |")
        lines.append("|---|---|---|---|---:|---:|---|")
        for block in matched_blocks:
            lines.append(
                "| {type} | {code} | {name} | {matched_keyword} | {change_pct} | {up}/{down} | {leader} |".format(
                    type=block.get("type", ""),
                    code=block.get("code", ""),
                    name=block.get("name", ""),
                    matched_keyword=block.get("matched_keyword", ""),
                    change_pct=block.get("change_pct", ""),
                    up=block.get("up_count", ""),
                    down=block.get("down_count", ""),
        ...
        lines.append("\n## Constituents")
        for block in matched_blocks:
            lines.append(f"\n### {block['type']}：{block['name']} ({block['code']})")
            try:
                stocks = _eastmoney_block_constituents(block["code"], limit=top_n)
            except Exception as e:
                lines.append(f"Constituent lookup failed: {e}")
                continue

            if not stocks:
                lines.append("No constituents returned.")
                continue

            lines.append("| 排名 | 代码 | 名称 | 涨跌幅 | 换手率 | PE | 主力净流入 | 行业 | 入池依据 |")
            lines.append("|---:|---|---|---:|---:|---:|---:|---|---|")
            for i, stock in enumerate(stocks, start=1):
                code = stock.get("code", "")
                duplicate_note = "重复板块成分" if code in seen_stocks else f"{block['name']} 成分股"
                seen_stocks.add(code)
                lines.append(
                    "| {rank} | {code} | {name} | {change_pct} | {turnover_pct} | {pe} | {main_net} | {industry} | {reason} |".format(
                        rank=i,
                        code=code,
                        name=stock.get("name", ""),
        ...
        lines.append("## Matched Blocks")
        lines.append("No Eastmoney block matched this keyword.")

    hot_matches = _hot_stock_keyword_matches(keyword, curr_date)
    if hot_matches:
        lines.append("\n## Hot-Stock Reason Matches")
        lines.append("# Fallback source: 同花顺涨停/强势股题材归因")
        for line in hot_matches:
            lines.append(f"- {line}")

    if not matched_blocks and not hot_matches:
        lines.append("\n## Next Step")
        lines.append("未找到可自动展开的板块或强势股题材。建议拆分为更短关键词，或改用行业/概念名/BK 板块代码。")

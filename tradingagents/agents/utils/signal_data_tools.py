...
def get_stocks_by_keyword(
    keyword: Annotated[
        str,
        "Keyword, concept, industry, region, or Eastmoney block code, e.g. 存储芯片 / 半导体设备 / 合肥 / BK1038",
    ],
    curr_date: Annotated[str, "Date in YYYY-MM-DD format, empty for today"] = "",
    ...
    """
    Reverse lookup A-share stock pools by keyword / concept / industry / region.
    First expands matched Eastmoney blocks into constituent stocks; if no block
    matches, falls back to today's hot-stock reason tags as discovery clues.
    Uses the configured signal_data vendor.
    Args:
        keyword: Keyword, concept, industry, region, or BK block code.
        curr_date: Date in YYYY-MM-DD format, empty string for today.
        top_n: Max constituent stocks per matched block.
        max_blocks: Max matched blocks to expand.
    Returns:
        str: Candidate stock pools with matched blocks, constituents, and fallback clues.
    """
    return route_to_vendor("get_stocks_by_keyword", keyword, curr_date, top_n, max_blocks)
